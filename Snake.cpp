...
Snake::Snake()
{
  ...
  SDL_CreateWindowAndRenderer(WIDTH_SCREEN, HEIGHT_SCREEN, SDL_WINDOW_BORDERLESS, &window, &renderer);
  SDL_CHECK(window, "SDL_CreateWindowAndRenderer");
  SDL_CHECK(renderer, "SDL_CreateWindowAndRenderer");
  SDL_SetWindowPosition(window, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED);
  auto surface = SDL_LoadBMP("snake.bmp");
  SDL_CHECK(surface, "SDL_LoadBMP(\"snake.bmp\")");
...
void Snake::generateFruit()
{
  ...
  do
  {
    fruitX = rand() % (WIDTH_SCREEN / 64);
    fruitY = rand() % (HEIGHT_SCREEN / 64);
    done = true;
...
int Snake::exec()
{
  ...
  for (auto done = false; !done;)
  {
    ...
    SDL_SetRenderDrawColor(renderer, 106, 59, 235, 255); // Color back ground
...
bool Snake::tick()
{
  if (ticks++ % 250 == 0)
  {
    ...
    p.first += dx;
    p.second += dy;
    if (p.first < 0 || p.first >= WIDTH_SCREEN / 64 ||
        p.second < 0 || p.second >= HEIGHT_SCREEN    / 64)
      return false;
    for (const auto &segment: segmentsList)
      if (p == segment)
        return false;
